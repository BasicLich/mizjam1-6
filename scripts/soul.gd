	...
	if body:
		...
		if body.is_in_group("headstone"):
			...
			if body.state == Game.POSSESSED:
				move = (body.global_position - global_position).normalized() * speed * 0.25
			else:
				move = (body.global_position - global_position).normalized() * speed * 0.5
			move_and_slide(move)
		...
		if Game.get_main().has_node(bodyName):
			body = Game.get_main().get_node(bodyName)
		else:
